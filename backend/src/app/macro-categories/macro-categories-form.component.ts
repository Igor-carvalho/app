...
@Component({
    templateUrl: './macro-categories-form.component.html',
})
export class MacroCategoriesFormComponent implements OnInit, OnDestroy {
    private _mode = '';

    private _id: number;
    private _parameters: any;
    private _object: MacroCategories;
    ...
    private _resetForm() {
        this._object = new MacroCategories();
        ...
        if (this._mode == 'create') {
            this._dataService.add(this._object)
                .subscribe(
                    result => {
                        if (result.success) {
                            this._router.navigate(['/macro-categories']);
                        } else {
